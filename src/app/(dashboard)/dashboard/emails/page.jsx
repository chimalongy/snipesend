  ...
  try {
    ...
    if (result.data.success) {
     ...
     await getUserEmails()
      ...
      toast.error("Failed to delete email:", result.data.message);
       //toast.error(result.data.message)
      
    }
  } catch (error) {
    console.error("Error deleting email:", error);
     toast.error("Error deleting email:", error)
  ...
  return (
    <div className="space-y-6 pb-[60px]">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <StatCard
          icon={<RiMailLine className="h-6 w-6 text-indigo-600" />}
          label="Total Email Accounts"
          value={emails.length}
          bgColor="bg-indigo-100"
        />
        <StatCard
          icon={<RiSendPlaneFill className="h-6 w-6 text-blue-600" />}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Daily Usage</th>
